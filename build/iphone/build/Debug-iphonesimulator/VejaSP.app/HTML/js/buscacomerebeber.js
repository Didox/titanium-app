...
VEJASP.Publicidade = {
  Render:function(){
    ...
    $("#bsc_resultado .attraction, #bsc_resultado .establishment").each(function(i,v){
      if(((i+1) % 3) === 0){
        j=j+1;
        $(v).append(app.getHtmlPublicidade("leaderboard"+j,"comerbeber"));
      }
    });
  }
};
...
VEJASP.Filtros = {
  showHideCombo:function(){
    $(VEJASP.BSC.navegadores).each(function(){
      ...
      } else if (this != 'anos_indicado_comer_beber' && this != 'anos_participante_comer_beber') {
       $(".bsc_nav_" + this).attr('style','display: none!important');
      }
    });
  },
  openCombo: {},
  ...
  printTerm:function(){
    ...
    var groupCategory = VEJASP.Filtros.getParameterByName("noite_grupo-meta_nav");
  ...
  openClosePertoDeMim: function(){
    ...
    if(window.location.href.indexOf("latitude") === -1){
      ...
      window.location.href = 'comerebeberbusca.html#&' + p.join('&');
    }
  },
  filterBomBarato: function(){
    var params = app.getParams();
    var p = [];
    if(app.notEmpty(params)){
      $.each(params, function(i, v){p.push(v);});
    }
    if(window.location.href.indexOf("bom_barato") === -1){
      window.location.href = 'comerebeberbusca.html#&' + p.join('&') + '&bom_barato=true';
    }else{
      window.location.href = window.location.href.replace('&bom_barato=true', '');
    }
  },
  filterMenuExecutivo: function(){
    var params = app.getParams();
    var p = [];
    if(app.notEmpty(params)){
      $.each(params, function(i, v){p.push(v);});
    }
    if(window.location.href.indexOf("menu_executivo") === -1){
      window.location.href = 'comerebeberbusca.html#&' + p.join('&') + '&menu_executivo=true';
    }else{
      window.location.href = window.location.href.replace('&menu_executivo=true', '');
    }
  },
  ...
  resultadoPertoDeMim:function(){
    if(window.location.hash.indexOf("latitude") != -1){
    ...
    if($(".bscNoResult b").text().indexOf("Food Truck") != -1){
      $(".bscNoResult span.perto-de-mim").hide();
    }
  },
  ...
  botoes: function(){
    ...
    if(typeof $("#bsc_paginador .bt-previous-page a").attr("href") != "undefined"){
      ...
      previousLink = previousLink.replace(/.*\/comerebeberbusca/,"comerebeberbusca").replace("#?","#");
    ...
    if(typeof $("#bsc_paginador .bt-next-page a").attr("href") != "undefined"){
      ...
      nextLink = nextLink.replace(/.*\/comerebeberbusca/g,"comerebeberbusca").replace("#?","#");
    ...
    if($("#bsc_navigators .buttons").size() == 0){
      ...
      if(window.location.hash.match(/category-meta_nav:Food%20Truck/) === null ){
        var comGps = location.hash.indexOf("latitude") !== -1 ? 'com-gps' : '';
        $("#bsc_navigators").prepend('<div class="buttonsTop"><a href="javascript:VEJASP.Filtros.openClosePertoDeMim()" class="botaoPertoDeMim '+comGps+'" onclick="app.eventAnalytics(\'Busca\',\'clique\',\'filtro-perto-de-mim\',0);">Perto de mim</a></div>');
      }

      if(window.location.hash.match(/(category-meta_nav:)(?!Food%20Truck)/) !== null ){
        var bomBaratoAtivo = location.hash.indexOf("&bom_barato") !== -1 ? 'bomBaratoAtivo' : '';
        $("#bsc_navigators .buttons").append('<a href="javascript:VEJASP.Filtros.filterBomBarato()" class="botaoBomBarato '+bomBaratoAtivo+'" onclick="app.eventAnalytics(\'Busca\',\'clique\',\'filtro-bom-e-barato\',0);">Bom e Barato</a>');
      }

      if(window.location.hash.match(/category-meta_nav:Restaurantes/) !== null ){
        var menuExecutivoAtivo = location.hash.indexOf("&menu_executivo") !== -1 ? 'menuExecutivoAtivo' : '';
        $("#bsc_navigators .buttons").append('<a href="javascript:VEJASP.Filtros.filterMenuExecutivo()" class="botaoMenuExecutivo '+menuExecutivoAtivo+'" onclick="app.eventAnalytics(\'Busca\',\'clique\',\'filtro-menu-executivo\',0);">Menu Executivo de até R$ 50</a>');
      }
...
VEJASP.BSC = {
  navegadores: ["sub_category", "neighborhood", "anos_vencedor_comer_beber", "preco_medio", "atracao_genero", "anos_indicado_comer_beber", "anos_participante_comer_beber", "editorial_rating"],
  ...
  setupCore:function(){
    ...
    $.extend(BSC, {
      buttonGenerate: function(v, link){
        ...
        clearLink = clearLink.replace(/.*\/comerebeberbusca/g,"comerebeberbusca");
        var prefix = '';
  ...
  setupNavigators:function(){
    ...
    BSC.createFilterEnderecosAvaliados = function(){
      var ano = '2014';

      $('.bsc_source_anos_vencedor_comer_beber').addClass('bsc_nav_enderecos_avaliados');
      $('.bsc_nav_enderecos_avaliados').hide();
      var bsc_nav_enderecos_avaliados = $('.bsc_nav_enderecos_avaliados').first();
      bsc_nav_enderecos_avaliados.show();

      $('#bsc_nav_anos_vencedor_comer_beber li a').each(function(){
        if($(this).text().indexOf(ano) >= 0){
          var li_vencedor = $(this).parent();
          var count = li_vencedor.text().match(/\([0-9]+\)/);
          $(this).text('Premiados ' + count);
          bsc_nav_enderecos_avaliados.children('ul').append(li_vencedor);
        } else {
          // $(this).hide();
        }
      });
      $("#bsc_nav_anos_vencedor_comer_beber li").each(function(){
    ...
    BSC.navigators = [
      ...
      this.createListSubNavigator('Vencedores','anos_vencedor_comer_beber','category'),
  ...
  setupCallbacks:function(){
    BSC.before(function(){
      if(window.location.hash.indexOf("Food%20Truck") != -1){
    ...
    BSC.after(function(){
      ...
      BSC.createFilterEnderecosAvaliados();
    });
  },
  ...
  ascPrecoMedio:function(){
    var range = [];
    $(".bsc_nav_preco_medio li").each(function(){ range.push($(this)); });
    $(".bsc_nav_preco_medio").html("");
    $(range.sort(function(a, b){
      return ($(b).text()) < ($(a).text()) ? 1 : -1;
    })).each(function(){
      $(".bsc_nav_preco_medio").append("<li>" + this.html() + "</li>");
    });
  },
  ...
  bscShowFiltros:function(){
    ...
    VEJASP.Filtros.hideEspecialidades();
...
BSC.ready(function(){
  ...
  BSC.fqParams.push("(anos_vencedor_comer_beber-meta_nav:\"2014\"%20OR%20anos_indicado_comer_beber-meta_nav:\"2014\"%20OR%20anos_participante_comer_beber-meta_nav:\"2014\")");

  if(location.hash.indexOf("bom_barato=true") !== -1){
    BSC.fqParams.push("(keywords-meta:\"bom e barato\")");
  }

  if(location.hash.indexOf("menu_exeuctivo=true") !== -1){
    BSC.fqParams.push("(keywords-meta:\"menu executivo\")");
  }
