...
VEJASP.Publicidade = {
  Render:function(){
    ...
    $("#bsc_resultado .attraction, #bsc_resultado .establishment").each(function(i,v){
      if(((i+1) % 3) === 0){
        j=j+1;
        $(v).append(app.getHtmlPublicidade("leaderboard"+j,"busca"));
      }
    });
  }
};
...
VEJASP.Filtros = {
  ...
  openClosePertoDeMim: function(){
    ...
    if(window.location.href.indexOf("latitude") === -1){
      ...
      window.location.href = 'cinema.html#&' + p.join('&');
    }
  },
  filterAvaliados: function(){
    ...
    var p = [];
      ...
      if(window.location.href.indexOf("avaliado_vejasp") === -1){
        window.location.href = 'cinema.html#&' + p.join('&') + '&avaliado_vejasp=true';
      }else{
  ...
  botoes: function(){
    ...
    if(typeof $("#bsc_paginador .bt-previous-page a").attr("href") != "undefined"){
      ...
      previousLink = previousLink.replace(/.*\/cinema/,"cinema").replace("#?","#");
    ...
    if(typeof $("#bsc_paginador .bt-next-page a").attr("href") != "undefined"){
      ...
      nextLink = nextLink.replace(/.*\/cinema/g,"cinema").replace("#?","#");
...
VEJASP.BSC = {
  navegadores: ["sub_category", "neighborhood", "anos_vencedor_comer_beber", "preco_medio", "atracao_genero"],
  ...
  setupCore:function(){
    ...
    $.extend(BSC, {
      buttonGenerate: function(v, link){
        ...
        clearLink = clearLink.replace(/.*\/cinema/g,"cinema");
        var prefix = '';
  ...
  setupNavigators:function(){
    ...
    $(document).on('click', 'a.bsc_ajax.selected', function(event){
      if(($this).text().indexOf('Cinemas ') != -1){
  ...
  setSelected:function(){
    $('a.bsc_ajax').filter(function(index) { if($(this).text().indexOf('Cinemas ') != -1){ $(this).addClass("selected").attr('href', location.href) } });
  },
  ...
  bscShowFiltros:function(){
    ...
    $(VEJASP.BSC.navegadores).each(function(){
      ...
      if(app.isAndroid()){
        // fixAndroid show
        if($("#header-filtro").css("display") && $("#header-filtro").css("display").indexOf("block") !== -1){
